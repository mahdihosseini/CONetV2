...
import copy
import numpy as np
def greedy_scaling_algorithm(dependency_list, conv_size_list, gamma, delta_scale, sel_metric, initial_step_size, min_conv_size, max_conv_size, trial_dir, cur_trial,mapping,global_config):
    
    # For first trial, initialize metrics and increase all channel size by initial_step_size
    if cur_trial == 0:
        # initialize
        ...
        return conv_size_list, cell_layer_avg_mM

    # record the mM from prev trial
    ...
    print('conv size metrics', conv_size_avg_mM_new)
    ...
    return conv_size_list, conv_size_avg_mM_new