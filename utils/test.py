...
def test_main(model, test_loader, epoch: int, device, optimizer, global_config) -> Tuple[float, float]:
    ...
    if acc > global_config.BEST_ACC:
        ...
        state = {
            ...
            'train_loss' : global_config.TRAIN_LOSS
        }
        if global_config.ADAS is not None:
            state['historical_io_metrics'] = global_config.METRICS.historical_metrics
        torch.save(state, os.path.join(global_config.CHECKPOINT_PATH, 'ckpt.pth'))
        # if checkpoint_path.is_dir():
        #     torch.save(state, str(checkpoint_path / 'ckpt.pth'))
        # else:
        #     torch.save(state, str(checkpoint_path))
        global_config.BEST_ACC = acc
    acc = acc / 100
    acc5 = acc5 / 100
    global_config.PERFORMANCE_STATISTICS[f'test_acc_epoch_{epoch}'] = acc
    global_config.PERFORMANCE_STATISTICS[f'test_acc5_epoch_{epoch}'] = acc5
    global_config.PERFORMANCE_STATISTICS[f'test_loss_epoch_{epoch}'] = \
